...
export const VacancyEventContainer: React.FC<VacancyEventContainerProps> = ({
  vacancy,
  consultantId,
}) => {
  const [editVacancy] = useMutation<
    EditVacancyPayload,
  ...
  const [deleteVacancy] = useMutation<number, { input: { id: number } }>(
    DELETE_VACANCY,
    {
      refetchQueries: [
        {
  ...
  return (
    <CalendarEvent
      ...
      render={
        <VacancyEventContent
          vacancy={vacancy}
          editCallback={editVacancyWrapper}
        />
      }
