...
import {
  GetConsultantCapacityPayload,
  GetConsultantVacancyPayload,
...
export const ConsultantCapacityRow: React.FC<ConsultantCapacityRowProps> =
  React.memo(({ consultantId }) => {
    const { data: cData } = useQuery<GetConsultantCapacityPayload>(
      GET_CONSULTANT_CAPACITY,
      {
        variables: {
      ...
      <CalendarRow
        sidebarContent={
          <CalendarSidebarHeader
            header={consultant?.firstName + " " + consultant?.lastName || "- -"}
          >
            ...
            >
              {cData && (
                <CalendarTimelineGrid>
                  {generateBoxesThatMatchCapacity(
                    cData.consultantsCapacity.items[0].capacity
                  )}
                </CalendarTimelineGrid>
              )}
            </span>
