...
import { DELETE_CONTRACT, GET_CONSULTANTS_INFO, GET_CONSULTANT_CAPACITY, GET_TEAMCONS_CONTRACTS } from '../../api/contract/queries';
...
export const DisplayConsultant: React.FC<ConsultantDisplayContainerProps> = ({ consultant }) => {
    ...
    const [deleteConsultant] = useMutation<number, { input: { id: number } }>(DELETE_CONSULTANT, {
        refetchQueries: [
            ...
            },
            {
                ...
                variables: { id: consultant.id },
            },
            {
                query: GET_CONSULTANT_CAPACITY,
                variables: { startYear: currentYear, endYear: currentYear + 2, id: consultant.id },
            },
        ],
        awaitRefetchQueries: true,
    });

    const [deleteProjectConsultant] = useMutation<number, { input: { id: number } }>(DELETE_PROJECTCONSULTANT, {
        refetchQueries: [
            {
                query: GET_CONSULTANTS_INFO
            },
    ...
    const deleteWrapper = (consultant: ConsultantWithContracts) => {
        deleteConsultant({ variables: { input: { id: consultant.id } } })
            .then((res) => {
                ...
                consultant.projectConsultants.forEach((project) => {
                    deleteProjectConsultant({ variables: { input: { id: project.id } } })
                        .then((res) => {
    ...
    return (
        <>
            <Box key={consultant.id} sx={{ display: 'flex', flexBasis: '100%', alignItems: 'center', height: '5vh', justifyContent: 'space-between', background: "#ffffff", boxShadow: '0px 0px 3px 0px rgba(0,0,0,0.1)', py: 2, px: 2, my: 1 }}>
                ...
                <Box>
                    <Button onClick={() => navigate(`/profile/${consultant.id}`)} sx={{ color: 'black', background: '#f2f6f8', ":hover": { background: '#cfd1d4' }, textTransform: 'none', fontWeight: '400', fontSize: '14px', display: 'flex', px: 2, py: 1, alignItems: 'center' }} size="small" disableRipple disableFocusRipple>
                        View Profile
                    </Button>
                </Box>
